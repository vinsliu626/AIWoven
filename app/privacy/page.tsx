...
> = {
    ...
    sections: [
      {
        ...
        items: [
          "Email address and account info (via OAuth providers)",
          "Usage data (feature usage counts, quotas)",
          "Device/browser metadata (for security and analytics)",
      ...
      },
      {
        ...
        items: [
          ...
          "Uploaded documents",
          "Detector input text",
          "Temporary conversion files after processing",
        ],
      },
      {
        title: "Temporary Processing",
        body: 'User inputs (audio, text, files, media) may be processed temporarily to generate results, but are not stored long-term unless explicitly required for the feature.',
      },
      {
        ...
        items: [
          ...
          "Stripe (billing)",
          "AI providers (Groq, OpenRouter, etc.)",
        ],
        footer: "These services may process data as necessary to provide functionality.",
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          "clear local data or saved browser preferences",
          "review policy information before using the product",
        ],
      },
      {
        title: "AI Disclaimer",
        items: [
          ...
          "NexusDesk does not replace professional, academic, legal, or medical advice.",
        ],
      },
      {
        title: "Children",
  ...
  zh: {
    ...
    summary: [
      ...
      "聊天、音频、文档、检测文本等输入大多只会被临时处理，不会长期保存。",
      "我们只收集运行产品、管理账户和执行额度限制所必需的信息。",
    ],
    sections: [
      {
        title: "我们收集的信息",
        items: [
          "邮箱地址和账户信息（通过 OAuth 登录提供）",
          "使用数据（功能使用次数、额度）",
          "设备 / 浏览器元数据（用于安全和分析）",
          "Cookie / 会话数据",
        ],
      ...
      {
        title: "临时处理",
        body: "音频、文本、文件和媒体等输入可能会被临时处理以生成结果；除非功能明确需要，否则不会长期保存。",
      },
      {
        title: "第三方服务",
        body: "我们会使用以下服务：",
        items: [
          ...
          "NextAuth（认证）",
          "Stripe（计费）",
          "AI providers（Groq、OpenRouter 等）",
        ],
        footer: "这些服务可能会在提供功能所必需的范围内处理相关数据。",
      },
      {
        title: "我们如何使用数据",
        body: "数据只会用于：",
